...
public final class OWLClassSubClassOfAxiomSolvability extends
     ...
     *        querySolver
     */
    public OWLClassSubClassOfAxiomSolvability(
    ...
    public SolvabilitySearchNode getSolvabilitySearchNode(OWLAxiom owlAxiom,
        ...
        return owlAxiom.accept(new OWLObjectVisitorEx<SolvabilitySearchNode>() {
            ...
            public SolvabilitySearchNode visit(OWLSubClassOfAxiom axiom) {
                ...
                boolean isSolvable = variableExtractor.extractVariables(axiom)
                        .size() == 1
                        && (variableExtractor.extractVariables(
                                axiom.getSuperClass()).isEmpty()
                                && !axiom.getSubClass().isAnonymous()
                                && variableExtractor.extractVariables(
                                        axiom.getSubClass()).size() == 1 || variableExtractor
                                .extractVariables(axiom.getSubClass())
                                .isEmpty()
                ...
                if (isSolvable) {
                    Variable<?> variable = variableExtractor
                            .extractVariables(axiom).iterator().next();
                    toReturn = variableExtractor.extractVariables(
                            axiom.getSubClass()).isEmpty() ? new SolvableSearchNode(
                            variable, axiom, bindingNode,
                            OWLClassSubClassOfAxiomSolvability.this
                                    .getQuerySolver().getSuperClasses(
                                            axiom.getSubClass()))
                            : new SolvableSearchNode(variable, axiom,
                                    bindingNode,
                                    OWLClassSubClassOfAxiomSolvability.this
