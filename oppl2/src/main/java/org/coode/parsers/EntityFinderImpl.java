...
public class EntityFinderImpl implements EntityFinder {
    ...
    @Override
    public Set<OWLClass>
            getMatchingOWLClasses(String match, boolean fullRegExp) {
    ...
    public Set<OWLNamedIndividual> getMatchingOWLIndividuals(String match) {
        return this.getEntities(match, OWLNamedIndividual.class,
                useRegularExpressions);
    }
    ...
    public Set<OWLDatatype> getMatchingOWLDataTypes(String match) {
        return this
                .getEntities(match, OWLDatatype.class, useRegularExpressions);
    }
     ...
     */
    private <T extends OWLEntity> Set<T> doWildcardSearch(String _match,
            Class<T> type) {
        String match = _match;
        ...
        if (match.startsWith(WILDCARD)) {
            ...
            matcher = (rendering, s) -> rendering.startsWith(s)
    ...
    private <T extends OWLEntity> Set<T> getAllEntities(Class<T> type) {
        ...
        manager.ontologies().forEach(o -> {
            if (type.equals(OWLClass.class)) {
                add(entities, o.classesInSignature());
            } else if (type.equals(OWLObjectProperty.class)) {
                add(entities, o.objectPropertiesInSignature());
            } else if (type.equals(OWLDataProperty.class)) {
                add(entities, o.dataPropertiesInSignature());
            } else if (type.equals(OWLIndividual.class)) {
                add(entities, o.individualsInSignature());
            } else if (type.equals(OWLDatatype.class)) {
                add(entities, o.datatypesInSignature());
            }
        });
