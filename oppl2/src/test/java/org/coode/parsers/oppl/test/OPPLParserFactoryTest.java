...
public class OPPLParserFactoryTest {
    ...
    private static OPPLScript parseScript(String opplString,
        ...
        OPPLParser parser = new ParserFactory(ontologyManager, testOntology,
                null).build(new JUnitTestErrorListener(), symbolTableFactory);
    ...
    public void shouldTestNewEntityCreationInline() {
        ...
        AnnotationBasedSymbolTableFactory annotationBasedSymbolTableFactory = new AnnotationBasedSymbolTableFactory(
                entire.getOWLOntologyManager(),
                Arrays.asList(OWLRDFVocabulary.RDFS_LABEL.getIRI()));
        OPPLScript parsedScript = parseScript(opplString,
                entire.getOWLOntologyManager(), entire,
